...
const CONNECTOR_OPTIONS = [
  ...
  { value: 'manual',           label: 'Manual / no interface' },
];
...
const CREDENTIAL_HELP = {
  ...
  'manual':           'No network connection is required. Jobs are started and completed by an operator on the printer detail page.',
};
      ...
      <section style={{ background: '#1e2433', borderRadius: 10, padding: 20, marginBottom: 24, maxWidth: 640 }}>
        ...
        <form onSubmit={handleAddPrinter}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 10 }}>
            ...
            <div>
              <label style={{ display: 'block', fontSize: 12, color: '#94a3b8', marginBottom: 4 }}>IP Address {addForm.type !== 'manual' && '*'}</label>
              <input
                value={addForm.ip}
                onChange={e => setAddForm(p => ({ ...p, ip: e.target.value }))}
                required={addForm.type !== 'manual'}
                disabled={addForm.type === 'manual'}
                placeholder={addForm.type === 'manual' ? 'Not required' : '192.168.1.100'}
                style={inputStyle}
              />
            </div>
